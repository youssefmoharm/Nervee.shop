...
**Test 2.4c: Dashboard Data Loads** 5. Verify dashboard shows:
...
```sql
-- Verify test discount codes exist from migrations 030 + 033
SELECT code, discount_type, discount_value, minimum_purchase, is_active, valid_until
FROM discount_codes
WHERE code IN ('WELCOME15', 'NERVE20', 'FREESHIP', 'BUNDLE10', 'COMEBACK10');
```

**Test 3.6a: Valid Code**

1. Cart total: EGP 1,200
2. Apply code `WELCOME15` (15% off, minimum purchase EGP 1,000)
3. Verify discount: EGP 180
4. Final total: EGP 1,020 + shipping

**Test 3.6b: Minimum Purchase Not Met**

1. Cart total: EGP 200
2. Apply code `NERVE20` (minimum purchase EGP 1,500)
...
**Test 3.6d: Server-Side Validation** 5. Apply valid code in UI 6. Inspect network request to `create-order` function 7. Verify discount is RE-VALIDATED server-side (not trusted from client)
...
**Test 4.1b: Edit Product** 6. Edit the product, change price 7. Save 8. Verify change reflected on storefront

**Test 4.1c: Delete Product** 9. Delete the product 10. Verify removed from storefront 11. Verify still exists in database (soft delete?) or actually deleted
...
**Test 4.4b: Test Code** 5. As customer, go to checkout with EGP 400 cart 6. Apply `TEST20` 7. Verify 20% discount applied

**Test 4.4c: Deactivate Code** 8. In admin, deactivate `TEST20` 9. Try to use as customer → should fail
...
**Test 4.5b: Trigger Notification** 4. As admin, edit that product 5. Set the sold-out size to in_stock = true, stock_quantity > 0 6. Save product
...
**Test 6.1b: Order Isolation** 5. User B should NOT be able to view User A's orders
...
**Test 6.3b: Tampered / Unauthorized Call** 4. Call without a token or with a non-admin token 5. Send to the function URL 6. Should REJECT with "Admin access required" (403) 7. Order should NOT be updated
...
**Test 8.1b: Duplicate Submission** 4. Double-click "Place Order" at checkout 5. Should create one order, not two (function validates and the order
number/id is unique)
...
- Checkout form is usable (COD — no payment iframe)
...
2. **Inventory forecasting**: No low-stock prediction, just current count
3. **Returns**: Policy documented and `request-return` exists, but no full self-service return dashboard
4. **Wishlist sharing**: shared-list pages exist (`031_wishlist_shares…`) but discovery is link-only (no public index)

Note: guest order lookup (`/guest-order` with email + order number + token),
GA4/Meta Pixel analytics, and wishlist sharing are **implemented** — they are
not limitations.
...
- **Playwright** (`npm run test:e2e`): local dev-server journeys, axe-core
  accessibility, cookie consent, contrast. Specs tagged `@live`
  (`tests/e2e/live-smoke.spec.ts`, `tests/e2e/security-idor.spec.ts`) hit
  the production site/backend and are **excluded by default**
  (`grepInvert` in `playwright.config.ts`). The nightly
  `.github/workflows/live-smoke.yml` sets `PLAYWRIGHT_LIVE=1` and runs only
  those specs against www.nerveey.shop.
