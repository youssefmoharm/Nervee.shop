...
serve(async req => {
  ...
  try {
    ...
    const rateLimitResult = await distributedRateLimit(supabase, `auth-sign-in:${identifier}`, {
      windowMs: 15 * 60 * 1000,
      maxRequests: 5,
    });
    ...
    // Attempt sign in — capture the session so we can return tokens for
    // supabase.auth.setSession(...) on the client (matches AuthContext contract).
    ...
    return json(
      {
        ...
        // Frontend may call supabase.auth.setSession({ access_token, refresh_token })
        // to establish the session in the browser client.
        session: session
          ? {
