...
serve(async req => {
  ...
  try {
    ...
    // Distributed rate limit: 5/min per IP (abuse protection for return spam)
    const ip = clientIp(req);
    const rateLimitResult = await distributedRateLimit(supabase, `request-return:${ip}`, {
      windowMs: 60000,
      maxRequests: 5,
    });
    ...
    if (insertErr) {
      ...
      timer.end();
      return json(
        { error: 'Failed to create request', details: insertErr.message },
        500,
        corsHeaders,
      );
    }
