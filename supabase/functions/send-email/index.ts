...
serve(async req => {
  ...
  try {
    ...
    // service_role: bucket by hashed key suffix (shared across internal callers).
    // Admin: bucket by client IP (never the bearer token itself).
    const rateId = isServiceRole ? `service:${token.slice(-8)}` : `ip:${clientIp(req)}`;
    const maxEmails = isServiceRole ? 200 : 20;
    const rateLimitResult = await distributedRateLimit(supabase, rateId, {
      windowMs: 60000,
    ...
    if (!ALLOWED_TYPES.has(emailType)) {
      timer.end();
      return json(
        { error: `Invalid email type. Allowed: ${[...ALLOWED_TYPES].join(', ')}` },
        400,
        corsHeaders,
      );
    }
    ...
    const unsubscribeToken = await supabase.rpc('create_unsubscribe_token', {
      p_email: body.to,
      p_email_type:
        emailType === 'transactional' || emailType === 'order_confirmation' ? null : emailType,
    });
    ...
    const sendResult = await sendEmail(body.to, body.subject.slice(0, 200), emailHtml, emailType);
    if (!sendResult.success) {
      // sendEmail already wrote a failed email_logs row (status/error_message).
      timer.end();
      return json({ error: 'Failed to send email', details: sendResult.error }, 502, corsHeaders);
    }
    ...
    return json(
      { success: true, message: 'Email sent successfully', to: body.to, type: emailType },
      200,
      corsHeaders,
    );
  } catch (err) {
