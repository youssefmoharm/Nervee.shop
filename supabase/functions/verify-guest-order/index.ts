...
serve(async req => {
  ...
  try {
    ...
    const ipLimit = await distributedRateLimit(supabase, `verify-guest-ip:${ip}`, {
      windowMs: 60000,
      maxRequests: 20,
    });
    if (!ipLimit.allowed) {
    ...
    if (!token || token.length < 8) {
      ...
      const emailLimit = await distributedRateLimit(supabase, `verify-guest-email:${email}`, {
        windowMs: 60000,
        maxRequests: 10,
      });
      if (!emailLimit.allowed) {
        timer.end();
        return json({ error: 'Too many attempts for this email.' }, 429, {
          ...corsHeaders,
    ...
    if (row.order_id) {
      const { data: fullOrder } = await supabase
        .from('orders')
        .select(
          'id, order_number, status, total, subtotal, shipping_cost, discount_amount, delivery_method, placed_at, created_at',
        )
        .eq('id', row.order_id)
        .maybeSingle();
