...
serve(async req => {
  ...
  try {
    ...
    if (!productId || !size)
      return json({ error: 'productId and size are required.' }, 400, corsHeaders);
    ...
    if (!inventory || inventory.stock_quantity <= 0) {
      return json(
        { notified: 0, message: 'This size has no stock — nothing to notify.' },
        200,
        corsHeaders,
      );
    }
    ...
    for (const request of requests) {
      ...
      // Only mark notified when the email actually went out — otherwise a
      // transient Resend failure would permanently skip the waitlist member.
      if (sendResult.success) {
        ...
        console.error(`Back-in-stock email failed for ${request.email}:`, sendResult.error);
      }
    }
    ...
    return json(
      { error: 'Something went wrong processing restock notifications.' },
      500,
      getCorsHeaders(req),
    );
  }
