...
/**
 * Send an email via Resend. Never throws — returns { success, error? } so
 * callers can decide whether to fail the request. On failure (including a
 * missing RESEND_API_KEY), a row is inserted into email_logs with status
 * 'failed' and the error message (best-effort; logging failure never blocks).
 *
 * @param type email type used for email_logs (matches send-email endpoint types)
 */
export async function sendEmail(
  ...
  if (!apiKey) {
    const msg = `RESEND_API_KEY not set — skipping email "${subject}" to ${to}`;
    console.warn(msg);
    await logEmailFailure(to, subject, type, msg);
  ...
  try {
    ...
    if (!res.ok) {
      ...
      await logEmailFailure(
        ...
        subject,
        type,
        `Resend API error (${res.status}): ${detail.slice(0, 500)}`,
      );
...
/**
 * Best-effort failure log into email_logs via a lazy service-role client.
 * Never throws.
 */
async function logEmailFailure(
  ...
  try {
    ...
    console.error('Failed to write email_logs failure row:', e);
  }
}
    ...
    ${items
      ...
      <tr style="border-bottom:1px solid #eee;">
        <td style="padding:10px 0;font-size:13px;">
          ...
          <span style="color:#71717a;">${escapeHtml(i.color)} / ${escapeHtml(i.size)} · Qty ${
          i.quantity
        }</span>
        </td>
...
function totalsTable(order: OrderForEmail): string {
  ...
  <table style="width:100%;font-size:13px;margin-top:8px;">
    ...
    <tr><td style="color:#71717a;padding:2px 0;">Shipping</td><td style="text-align:right;">${
      order.shipping_cost === 0 ? 'Free' : `EGP ${order.shipping_cost}`
    }</td></tr>
    ${
      order.discount_amount > 0
        ? `<tr><td style="color:#71717a;padding:2px 0;">Discount</td><td style="text-align:right;">-EGP ${order.discount_amount.toLocaleString()}</td></tr>`
        : ''
    }
...
export function orderConfirmedEmail(
  ...
  const trackingBlock = trackingLink
    ? `<p style="font-size:13px;color:#71717a;margin-top:16px;">Track your order: <a href="${trackingLink}" style="color:#061735;">${trackingLink}</a></p>`
    : '';
  return layout(
    `Order #${order.order_number} confirmed`,
    `
    <h1 style="font-size:20px;margin:0 0 8px;">Order Confirmed</h1>
    <p style="font-size:14px;color:#3f3f46;">Hi ${escapeHtml(
      order.first_name,
    )}, thanks for your order — we're getting it ready.</p>
    <p style="font-size:13px;color:#71717a;margin:16px 0 4px;">Order #${order.order_number}</p>
    ${itemsTable(items)}
    ${totalsTable(order)}
    <p style="font-size:13px;color:#71717a;margin-top:20px;">Shipping to: ${escapeHtml(
      order.address,
    )}, ${escapeHtml(order.city)}, ${escapeHtml(order.governorate)}</p>
    ${trackingBlock}
    `,
  );
}
  ...
  return layout(
    ...
    ${
      order.tracking_number
        ? `<p style="font-size:13px;margin:16px 0;">Tracking number: <strong>${escapeHtml(
            order.tracking_number,
          )}</strong>${
            order.tracking_url
              ? ` — <a href="${order.tracking_url}" style="color:#061735;">Track your package</a>`
              : ''
          }</p>`
        : ''
    }
...
export function orderCancelledEmail(order: OrderForEmail) {
  return layout(
    ...
    <p style="font-size:14px;color:#3f3f46;">Hi ${escapeHtml(order.first_name)}, order #${
      order.order_number
    } has been cancelled. Any reserved items have been released back into stock.</p>
...
export function backInStockEmail(productName: string, size: string, url: string) {
  return layout(
    ...
    <p style="font-size:14px;color:#3f3f46;">${escapeHtml(
      productName,
    )} is back in stock in size ${escapeHtml(size)} — while it lasts.</p>
