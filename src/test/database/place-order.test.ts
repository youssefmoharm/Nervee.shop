...
async function mockPlaceOrder(params: any) {
  ...
  for (const item of p_items) {
    ...
    if (!product) {
      return {
        data: null,
        error: { message: `Product ${item.product_id} is no longer available` },
      };
    }
    ...
    if (inventory < item.quantity) {
      return {
        data: null,
        error: { message: `${item.product_id} (size ${item.size}) only has ${inventory} in stock` },
      };
    }
  }
  ...
  if (p_discount_code) {
    ...
    if (
      discountCode &&
      discountCode.active &&
      discountCode.usageCount < (discountCode.usageLimit || Infinity)
    ) {
