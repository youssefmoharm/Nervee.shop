        ...
        eq: (column: string, value: any) => ({
          single: async () => {
            ...
            if (userType === 'anonymous') {
              ...
              if (['orders', 'customers', 'customer_addresses', 'carts', 'cart_items', 'wishlists', 'wishlist_items', 'payment_events'].includes(table)) {
              ...
              switch (table) {
                ...
                case 'wishlist_items':
                  ...
                  filteredData = tableData.filter((row: any) => userWishlistIds.includes(row.wishlist_id))
                  break
                case 'payment_events':
                  return { data: null, error: { message: 'Access denied' } }
                case 'admin_users':
...
describe('Row Level Security (RLS)', () => {
  describe('Anonymous User Access', () => {
    ...
    it('blocks access to payment events', async () => {
      const result = await anonClient.from('payment_events').select().limit(1).then()
      expect(result.data).toEqual([])
    })
  })
  ...
  describe('Customer Data Isolation', () => {
    ...
    it('blocks customer from viewing other customer orders', async () => {
      const result = await customer2Client.from('orders').select().eq('customer_id', 'customer-1').single()
      expect(result.data).toBeNull()
    })

    it('allows customer to view own profile only', async () => {
      const result = await customer1Client.from('customers').select().eq('id', 'customer-1').single()
    ...
    it('blocks customer from viewing other customer profile', async () => {
      const result = await customer1Client.from('customers').select().eq('id', 'customer-2').single()
      expect(result.data).toBeNull()
    })
  ...
  describe('Admin Status Check', () => {
    ...
    it('blocks customer access to payment events', async () => {
      const result = await customerClient.from('payment_events').select().limit(1).then()
      expect(result.data).toEqual([])
    })

    it('allows user to check own admin status only', async () => {
      const result = await customerClient.from('admin_users').select().eq('user_id', 'customer-1').single()
