...
export const guestOrderService = {
    ...
    async lookup(email: string, orderNumber: string, verificationToken?: string) {
        ...
        try {
            let query = supabase.from('guest_orders').select('*').eq('email', email).eq('order_number', orderNumber)
            ...
            console.error('guestOrderService.lookup failed:', err)
    ...
    async create(email: string, orderNumber: string, verificationToken: string) {
        ...
        try {
            ...
            console.error('guestOrderService.create failed:', err)
    ...
    async sendVerificationLink(email: string, orderNumber: string, verificationToken: string) {
        ...
        try {
            ...
            console.error('guestOrderService.sendVerificationLink failed:', err)
            return { success: false, error: 'Network error. Please try again.' }
        }
    },
}
