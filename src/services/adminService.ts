...
export const adminService = {
  async getDashboardStats() {
    const [{ count: orderCount }, { count: customerCount }, { data: orders }, { data: lowStock }] = await Promise.all([
  ...
  async updateOrderStatus(orderId: string, status: string, tracking?: { trackingNumber?: string; trackingUrl?: string }) {
  ...
  /** Re-checks Paymob directly for orders whose webhook may never have arrived. */
  async verifyPayment(orderId: string) {
    const { data, error } = await supabase.functions.invoke('verify-payment', { body: { orderId } })
    if (error) return { message: null, error: error.message }
    if (data?.error) return { message: null, error: data.error as string }
    return { message: data.message as string, error: null }
  },

  /** Notifies anyone waiting on this product/size once it's back in stock. */
  async triggerRestockCheck(productId: string, size: string) {
    const { data, error } = await supabase.functions.invoke('process-restock', { body: { productId, size } })
    if (error) {
      console.error('process-restock failed:', error)
      return
    }
    if (data?.notified) console.info(`Notified ${data.notified} customer(s) that ${productId} (${size}) is back in stock.`)
  },
  ...
  async runPaymentReconciliation() {
    const { data, error } = await supabase.functions.invoke('payment-reconciliation', { body: {} })
    if (error) return { error: error.message }
    return { data }
  },

  async createDiscount(discount: Record<string, unknown>) {
    const { error } = await supabase.from('discount_codes').insert({ ...discount, code: String(discount.code).toUpperCase() })
    return { error: error?.message ?? null }
  },
