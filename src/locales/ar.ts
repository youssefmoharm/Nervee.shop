/**
 * Arabic UI strings. Keys are the English source strings (en fallback).
 * Product names / user content are intentionally not translated.
 */
export const ar: Record<string, string> = {
  // Chrome / nav
  Shop: 'المتجر',
  'New Drop': 'وصل حديثًا',
  Collections: 'التشكيلات',
  About: 'عننا',
  Contact: 'اتصل بنا',
  Newsletter: 'النشرة البريدية',
  Shipping: 'الشحن',
  Returns: 'الإرجاع',
  FAQ: 'الأسئلة الشائعة',
  'Size Guide': 'دليل المقاسات',
  Privacy: 'الخصوصية',
  Terms: 'الشروط',
  Help: 'المساعدة',
  'Skip to main content': 'تخطَّ إلى المحتوى الرئيسي',
  Search: 'بحث',
  Account: 'حسابي',
  Menu: 'القائمة',
  'Close menu': 'إغلاق القائمة',
  'Alexandria, Egypt': 'الإسكندرية، مصر',
  'All rights reserved': 'جميع الحقوق محفوظة',

  // Cart
  'Your Bag': 'حقيبتك',
  'Your bag is empty.': 'حقيبتك فارغة.',
  'Your bag is empty': 'حقيبتك فارغة',
  'Shop the Drop': 'تسوق الإطلاق',
  'Continue Shopping': 'متابعة التسوق',
  Subtotal: 'المجموع الفرعي',
  Discount: 'الخصم',
  'Estimated Shipping': 'الشحن المتوقع',
  Free: 'مجاني',
  Total: 'الإجمالي',
  'Order Summary': 'ملخص الطلب',
  'Discount code': 'كود الخصم',
  Apply: 'تطبيق',
  'Proceed to Checkout': 'إتمام الشراء',
  'Decrease quantity': 'إنقاص الكمية',
  'Increase quantity': 'زيادة الكمية',
  'Remove item': 'إزالة المنتج',
  'You May Also Like': 'قد يعجبك أيضًا',
  'Free shipping on orders over': 'شحن مجاني للطلبات فوق',
  'Browse the drop and add pieces you love — checkout is cash on delivery across Egypt.':
    'تصفّح الإطلاق وأضف القطع التي تحبها — إتمام الشراء بالدفع عند الاستلام في جميع أنحاء مصر.',

  // Checkout
  Checkout: 'إتمام الشراء',
  Information: 'المعلومات',
  Delivery: 'التوصيل',
  Payment: 'الدفع',
  Confirmation: 'التأكيد',
  Email: 'البريد الإلكتروني',
  'First Name': 'الاسم الأول',
  'Last Name': 'اسم العائلة',
  Phone: 'الهاتف',
  'Phone Number': 'رقم الهاتف',
  Address: 'العنوان',
  City: 'المدينة',
  Governorate: 'المحافظة',
  'Postal Code': 'الرمز البريدي',
  'Standard Delivery': 'توصيل عادي',
  'Express Delivery': 'توصيل سريع',
  'Payment Method': 'طريقة الدفع',
  'Cash on Delivery': 'الدفع عند الاستلام',
  'Delivery & Payment': 'التوصيل والدفع',
  'Pay to the courier when your order arrives': 'ادفع للمندوب عند وصول طلبك',
  'Online card payment is not available — every order is cash on delivery.':
    'الدفع بالبطاقة غير متاح — جميع الطلبات الدفع عند الاستلام.',
  'Review Your Order': 'مراجعة طلبك',
  'Place Order': 'تقديم الطلب',
  'Placing order…': 'جارٍ تقديم الطلب…',
  Back: 'رجوع',
  Next: 'التالي',
  'Shipping Address': 'عنوان الشحن',
  'Customer Information': 'بيانات العميل',
  'Continue to Next Step': 'متابعة إلى الخطوة التالية',
  'Back to bag': 'العودة إلى الحقيبة',
  'Please review all details before placing your order. You can go back to make any changes.':
    'راجع جميع التفاصيل قبل تقديم الطلب. يمكنك الرجوع للتعديل.',
  'Order Protected': 'طلبك محمي',
  'We keep your information secure and only use it to fulfill your order and send delivery updates.':
    'نحافظ على أمان معلوماتك ونستخدمها فقط لتنفيذ طلبك وإرسال تحديثات التوصيل.',
  'We currently deliver across Egypt with the fastest available option for your governorate.':
    'نوصّل حاليًا إلى جميع أنحاء مصر مع أسرع خيار متاح لمحافظتك.',
  'Choose your delivery speed and preferred payment method. We deliver across Egypt with the fastest available option for your governorate.':
    'اختر سرعة التوصيل وطريقة الدفع المفضلة. نوصّل إلى جميع أنحاء مصر مع أسرع خيار متاح لمحافظتك.',

  // Auth
  Login: 'تسجيل الدخول',
  Register: 'إنشاء حساب',
  'Forgot Password?': 'نسيت كلمة المرور؟',
  'Reset Password': 'إعادة تعيين كلمة المرور',
  Password: 'كلمة المرور',
  'Confirm Password': 'تأكيد كلمة المرور',
  'Sign In': 'تسجيل الدخول',
  'Sign Up': 'إنشاء حساب',
  'Check Your Email': 'تحقق من بريدك الإلكتروني',
  'First name': 'الاسم الأول',
  'Last name': 'اسم العائلة',
  'Create Account': 'إنشاء حساب',
  'Forgot password?': 'نسيت كلمة المرور؟',
  'Create an account': 'أنشئ حسابًا',
  'Sign in': 'تسجيل الدخول',
  'Sign in to your account': 'سجّل الدخول إلى حسابك',
  'Back to Sign In': 'العودة لتسجيل الدخول',
  'Date of Birth': 'تاريخ الميلاد',
  Gender: 'الجنس',
  'Track Order': 'تتبع الطلب',
  'Sending…': 'جارٍ الإرسال…',
  'Must be 8+ characters with uppercase, lowercase, number, and special character.':
    'يجب أن تكون 8 أحرف على الأقل مع حرف كبير وحرف صغير ورقم ورمز خاص.',
  'Password must contain at least one uppercase letter.':
    'يجب أن تحتوي كلمة المرور على حرف كبير واحد على الأقل.',
  'Password must contain at least one lowercase letter.':
    'يجب أن تحتوي كلمة المرور على حرف صغير واحد على الأقل.',
  'Password must contain at least one number.': 'يجب أن تحتوي كلمة المرور على رقم واحد على الأقل.',
  'Password must contain at least one special character.':
    'يجب أن تحتوي كلمة المرور على رمز خاص واحد على الأقل.',
  'Please enter your date of birth.': 'يرجى إدخال تاريخ ميلادك.',
  'Please select your gender.': 'يرجى اختيار جنسك.',
  'Already have an account?': 'لديك حساب بالفعل؟',
  'Profile Photo (optional)': 'صورة الملف الشخصي (اختياري)',
  'We sent a confirmation link to': 'أرسلنا رابط تأكيد إلى',
  'Verify your email, then sign in to your account.':
    'تحقّق من بريدك الإلكتروني ثم سجّل الدخول إلى حسابك.',

  // Tracking
  'Track Your Order': 'تتبع طلبك',
  'Order Tracking': 'تتبع الطلب',
  'Track your NERVE order below': 'تتبع طلب NERVE أدناه',
  'Order Number': 'رقم الطلب',
  'Look Up Order': 'البحث عن الطلب',
  'Resend verification link': 'إعادة إرسال رابط التحقق',
  'Order Placed': 'تم استلام الطلب',
  Processing: 'قيد المعالجة',
  Shipped: 'تم الشحن',
  Delivered: 'تم التوصيل',
  Cancelled: 'ملغي',
  Refunded: 'تم الاسترداد',

  // Product / PDP
  'Add to Bag': 'أضف إلى الحقيبة',
  'Quick Add': 'إضافة سريعة',
  'Quick View': 'عرض سريع',
  Compare: 'مقارنة',
  'In Comparison': 'في المقارنة',
  'Find My Size': 'اعثر على مقاسي',
  'Low Stock': 'مخزون منخفض',
  Helpful: 'مفيد',
  'Write a review': 'اكتب تقييمًا',
  'Complete the Look': 'أكمل الإطلالة',
  'Bundle Summary': 'ملخص الباقة',
  'Add Bundle to Bag': 'أضف الباقة إلى الحقيبة',
  'Regular price': 'السعر العادي',
  'Bundle Price': 'سعر الباقة',

  // Wishlist / share
  Wishlist: 'المفضلة',
  Share: 'مشاركة',
  'Copy link': 'نسخ الرابط',
  'Link copied': 'تم نسخ الرابط',

  // Common
  Loading: 'جارٍ التحميل',
  Save: 'حفظ',
  Cancel: 'إلغاء',
  Delete: 'حذف',
  Edit: 'تعديل',
  Close: 'إغلاق',
  Optional: 'اختياري',
  Required: 'مطلوب',
  'Free shipping': 'شحن مجاني',
  'Cash on delivery across Egypt': 'الدفع عند الاستلام في جميع أنحاء مصر',
  'Contemporary Egyptian streetwear. Cool but chic — Alexandria, Egypt.':
    'أزياء شارع مصرية معاصرة. أنيق لكن أنيق — الإسكندرية، مصر.',

  // Newly wired UI strings
  ', a reset link is on its way.': '، رابط إعادة التعيين في طريقه إليك.',
  '⏱️ Recent Searches': 'عمليات البحث الأخيرة',
  '✓ In Stock': '✓ متوفر',
  '✓ Order Protected': '✓ طلبك محمي',
  '✓ Verified': '✓ موثّق',
  '✓ Verified Purchase': '✓ عملية شراء موثّقة',
  '✓ Your order qualifies for free shipping!': '✓ طلبك مؤهل للشحن المجاني!',
  '✗ Out of Stock': '✗ غير متوفر',
  '✨ New': '✨ جديد',
  '1. The Basics': '1. الأساسيات',
  '1-2 business days': '1-2 أيام عمل',
  '2 hours': 'ساعتان',
  '2. Prices & Currency': '2. الأسعار والعملة',
  '2-5 business days': '2-5 أيام عمل',
  '3. Stock & Orders': '3. المخزون والطلبات',
  '30-day easy returns': 'إرجاع سهل خلال 14 يومًا',
  '3-5 business days': '2-5 أيام عمل',
  "4. Don't Steal Our Vibe (Intellectual Property)": '4. لا تسرق أجواءنا (الملكية الفكرية)',
  '5. Colors & Reality': '5. الألوان والواقع',
  '6. Policy Updates': '6. تحديثات السياسة',
  '🔥 Trending': '🔥 رائج',
  "🔥 What's Hot": '🔥 الأكثر رواجًا',
  '🔍 Suggestions': '🔍 اقتراحات',
  '📸 With Photos': '📸 مع صور',
  'About Me': 'نبذة عني',
  'About NERVE': 'عن NERVE',
  'Accept all': 'قبول الكل',
  'Account Settings': 'إعدادات الحساب',
  Action: 'إجراء',
  Add: 'إضافة',
  'Add a personal message (optional)': 'أضف رسالة شخصية (اختياري)',
  'Add a piece before checking out — payment is cash on delivery across Egypt.':
    'أضف قطعة قبل إتمام الشراء — الدفع عند الاستلام في جميع أنحاء مصر.',
  'Add Address': 'إضافة عنوان',
  'Add items to your wishlist first': 'أضف منتجات إلى المفضلة أولاً',
  'Add Photos (optional, max 3)': 'أضف صورًا (اختياري، بحد أقصى 3)',
  'Add to bundle': 'إضافة إلى الباقة',
  'Add to wishlist': 'إضافة إلى المفضلة',
  'Added to comparison': 'تمت الإضافة إلى المقارنة',
  'Added to wishlist': 'تمت الإضافة إلى المفضلة',
  Addresses: 'العناوين',
  'Alexandria, Egypt · EST 2026': 'الإسكندرية، مصر · تأسست 2026',
  All: 'الكل',
  'All Reviews': 'جميع التقييمات',
  'and cash on delivery across Egypt.': 'والدفع عند الاستلام في جميع أنحاء مصر.',
  'and secure payment protection.': 'وحمية الدفع الآمنة.',
  'applied — you saved.': 'تم التطبيق — وفّرت.',
  'Applied to all orders within Egypt': 'يُطبق على جميع الطلبات داخل مصر',
  'Applying...': 'جارٍ التطبيق...',
  'At least 8 characters': '8 أحرف على الأقل',
  Authenticity: 'الأصالة',
  'Back to Collections': 'العودة إلى التشكيلات',
  'Back to Home': 'العودة إلى الرئيسية',
  'Back to orders': 'العودة إلى الطلبات',
  Badge: 'شارة',
  'Browse Products': 'تصفح المنتجات',
  'Buy Now': 'اشترِ الآن',
  'Calculate My Size': 'احسب مقاسي',
  cancel: 'إلغاء',
  'Cancel Order': 'إلغاء الطلب',
  'Cancel Order Now': 'إلغاء الطلب الآن',
  Cancellation: 'إلغاء',
  Care: 'العناية',
  'Care Instructions:': 'تعليمات العناية:',
  'Cash on delivery available': 'الدفع عند الاستلام متاح',
  Category: 'الفئة',
  Centimeters: 'سنتيمترات',
  'Change Password': 'تغيير كلمة المرور',
  'Changed your mind? You can resubscribe anytime:': 'تغيّر رأيك؟ يمكنك الاشتراك مجددًا في أي وقت:',
  'Changing Password...': 'جارٍ تغيير كلمة المرور...',
  characters: 'حرفًا',
  'Checking...': 'جارٍ التحقق...',
  Chest: 'الصدر',
  'Chest Measurement': 'قياس الصدر',
  Clear: 'مسح',
  'Clear All': 'مسح الكل',
  'Clear all filters': 'مسح جميع الفلاتر',
  'Clear search': 'مسح البحث',
  'Close bag': 'إغلاق الحقيبة',
  'Close comparison modal backdrop': 'إغلاق خلفية نافذة المقارنة',
  'Close modal': 'إغلاق النافذة',
  'Close overlay': 'إغلاق الطبقة',
  'Close search': 'إغلاق البحث',
  'Close size guide': 'إغلاق دليل المقاسات',
  Code: 'الكود',
  'Collection Not Found': 'التشكيل غير موجود',
  Color: 'اللون',
  Colors: 'الألوان',
  Comment: 'التعليق',
  'Compare Products': 'مقارنة المنتجات',
  'Compare up to 3 products side-by-side to make the best choice':
    'قارن حتى 3 منتجات جنبًا إلى جنب لاتخاذ أفضل قرار',
  'Complete Your Style': 'أكمل أسلوبك',
  'Confirm New Password': 'تأكيد كلمة المرور الجديدة',
  'Confirm your new password': 'أعد كتابة كلمة المرور الجديدة',
  'Connection Error': 'خطأ في الاتصال',
  'Connection error - try again': 'خطأ في الاتصال - حاول مرة أخرى',
  'Contact Information': 'بيانات التواصل',
  'Contact Support': 'تواصل مع الدعم',
  'Contact Us': 'تواصل معنا',
  'Cookie consent': 'موافقة ملفات تعريف الارتباط',
  Cookies: 'ملفات تعريف الارتباط',
  'Copied!': 'تم النسخ!',
  'Copy to clipboard': 'نسخ إلى الحافظة',
  'Could not add bundle. Please try again.': 'تعذّرت إضافة الباقة. حاول مرة أخرى.',
  'Creating link…': 'جارٍ إنشاء الرابط…',
  'Curated Edits': 'تنسيقات مختارة',
  'Current Password': 'كلمة المرور الحالية',
  'Current password is incorrect': 'كلمة المرور الحالية غير صحيحة',
  Customer: 'العميل',
  'Customer Focus': 'تركيز العميل',
  'Customer Reviews': 'تقييمات العملاء',
  Default: 'افتراضي',
  'Delete address': 'حذف العنوان',
  'Delivery Information': 'معلومات التوصيل',
  'Delivery Method': 'طريقة التوصيل',
  'Delivery time': 'مدة التوصيل',
  'Desired Length': 'الطول المطلوب',
  'Did you mean:': 'هل تقصد:',
  'Discount Applied': 'تم تطبيق الخصم',
  'Discount Code': 'كود الخصم',
  Discover: 'اكتشف',
  Done: 'تم',
  "Don't have an order number?": 'ليس لديك رقم طلب؟',
  'Early access to new collections': 'وصول مبكر إلى التشكيلات الجديدة',
  'Email address': 'البريد الإلكتروني',
  'Email Address': 'البريد الإلكتروني',
  'Email cannot be changed': 'لا يمكن تغيير البريد الإلكتروني',
  'Enter a strong password': 'أدخل كلمة مرور قوية',
  'Enter a valid email to join the list.': 'أدخل بريدًا إلكترونيًا صالحًا للانضمام إلى القائمة.',
  'Enter code': 'أدخل الكود',
  'ENTER CODE': 'أدخل الكود',
  'Enter the email address and order number from your confirmation email.':
    'أدخل البريد الإلكتروني ورقم الطلب من رسالة تأكيد طلبك.',
  'Enter your current password': 'أدخل كلمة المرور الحالية',
  'Enter your email': 'أدخل بريدك الإلكتروني',
  "Enter your email and we'll send you a link to reset your password.":
    'أدخل بريدك الإلكتروني وسنرسل لك رابطًا لإعادة تعيين كلمة المرور.',
  'Enter your first name': 'أدخل اسمك الأول',
  'Enter your last name': 'أدخل اسم عائلتك',
  'Enter Your Measurements': 'أدخل قياساتك',
  'Essential only': 'الضرورية فقط',
  'Exclusive offers and discounts': 'عروض وخصومات حصرية',
  'Explore NERVE': 'استكشف NERVE',
  'Express delivery available': 'التوصيل السريع متاح',
  'Failed to load products': 'فشل تحميل المنتجات',
  'Failed to submit review': 'فشل إرسال التقييم',
  'Failed to update profile': 'فشل تحديث الملف الشخصي',
  'Fashion inspiration and styling tips': 'إلهام أزياء ونصائح تنسيق',
  'Fast delivery updates': 'تحديثات توصيل سريعة',
  Female: 'أنثى',
  'Find something you love and it will show up here.': 'ابحث عن قطعة تحبها وستظهر هنا.',
  'Find your perfect fit with our comprehensive size charts and measurement guide':
    'اعثر على مقاسك المثالي مع جداول المقاسات ودليل القياس الشامل',
  'Find Your Size': 'اعثر على مقاسك',
  Fit: 'القصة',
  'Fit Guide': 'دليل القصات',
  'Free delivery over': 'توصيل مجاني فوق',
  'Free returns within 14 days': 'إرجاع مجاني خلال 14 يومًا',
  'Free returns within 14 days of delivery on unworn items.':
    'إرجاع مجاني خلال 14 يومًا من التوصيل للمنتجات غير المستخدمة.',
  'Free Shipping at': 'شحن مجاني عند',
  'Free Shipping Qualified': 'مؤهل للشحن المجاني',
  'Frequently Asked Questions': 'الأسئلة الشائعة',
  Filters: 'عوامل التصفية',
  'Open filters': 'فتح عوامل التصفية',
  'Close filters': 'إغلاق عوامل التصفية',
  'Show results': 'عرض النتائج',
  'from our email list.': 'من قائمتنا البريدية.',
  'Generate Share Link': 'إنشاء رابط المشاركة',
  'Get early access to new drops, exclusive releases, and special offers.':
    'احصل على وصول مبكر للإصدارات الجديدة والإطلاقات الحصرية والعروض الخاصة.',
  'Get In Touch': 'تواصل معنا',
  'Get in Touch Directly': 'تواصل مباشرة',
  'Get Size Recommendation': 'احصل على توصية المقاس',
  'Grid view': 'عرض شبكي',
  'Have a discount code?': 'لديك كود خصم؟',
  'Highest Rating': 'الأعلى تقييمًا',
  Home: 'الرئيسية',
  'How to Measure': 'كيف تقيس',
  'How to measure:': 'كيف تقيس:',
  'How we use it': 'كيف نستخدمه',
  'If an account exists for': 'إذا كان هناك حساب لـ',
  'If that email exists, a link is on its way.': 'إذا كان هذا البريد موجودًا، فالرابط في طريقه.',
  Inches: 'إنش',
  'Invalid code. Please check and try again.': 'كود غير صالح. تحقق وحاول مرة أخرى.',
  'Invalid discount code.': 'كود خصم غير صالح.',
  'Invalid Link': 'رابط غير صالح',
  'is back.': 'عاد.',
  "is out of stock. Get an email when it's back:": 'غير متوفر. احصل على إشعار عند توفره:',
  items: 'قطعة',
  'Join NERVE for faster checkout and order tracking.':
    'انضم إلى NERVE لتسريع إتمام الشراء وتتبع الطلبات.',
  'Join Our Newsletter': 'انضم إلى نشرتنا البريدية',
  'Joining…': 'جارٍ الانضمام…',
  'Just Landed': 'وصل للتو',
  'Label (e.g. Home, Work)': 'التسمية (مثل: المنزل، العمل)',
  Length: 'الطول',
  'Like what you see?': 'يعجبك ما تراه؟',
  'Link copied to clipboard!': 'تم نسخ الرابط إلى الحافظة!',
  'List view': 'عرض قائمة',
  'Load More': 'عرض المزيد',
  'Loading wishlist...': 'جارٍ تحميل المفضلة...',
  'Loading…': 'جارٍ التحميل…',
  'Lowercase letter (a-z)': 'حرف صغير (a-z)',
  Male: 'ذكر',
  Material: 'الخامة',
  'Material Care Tips': 'نصائح العناية بالخامة',
  'Material Information': 'معلومات الخامة',
  'Material:': 'الخامة:',
  'Max Price': 'أقصى سعر',
  'Maximum price': 'الحد الأقصى للسعر',
  'measure around the fullest part of your chest, keeping the tape level.':
    'قِس حول أوسع جزء من صدرك مع إبقاء الشريط أفقيًا.',
  'measure around your natural waistline.': 'قِس حول خصرك الطبيعي.',
  'measure from the highest point of the shoulder to the hem.':
    'قِس من أعلى نقطة في الكتف حتى الذيل.',
  Message: 'الرسالة',
  "Message sent — we'll get back to you shortly.": 'تم إرسال الرسالة — سنرد عليك قريبًا.',
  'more for FREE shipping': 'المزيد للشحن المجاني',
  'More Questions': 'المزيد من الأسئلة',
  'Most Helpful': 'الأكثر فائدة',
  Name: 'الاسم',
  'Need help?': 'تحتاج مساعدة؟',
  'Need to': 'تحتاج',
  'Nerve in the Wild': 'NERVE في الشارع',
  'New Password': 'كلمة المرور الجديدة',
  'Newest First': 'الأحدث أولًا',
  'Next image': 'الصورة التالية',
  'No products available yet. Check back soon.': 'لا توجد منتجات بعد. عد قريبًا.',
  'No products in this collection yet.': 'لا توجد منتجات في هذا التشكيل بعد.',
  'No products match that search': 'لا توجد منتجات مطابقة لهذا البحث',
  'No Products to Compare': 'لا توجد منتجات للمقارنة',
  'No results for': 'لا نتائج لـ',
  'No reviews yet. Be the first to review!': 'لا تقييمات بعد. كن أول من يقيّم!',
  'No saved addresses yet.': 'لا توجد عناوين محفوظة بعد.',
  'No spam, unsubscribe anytime': 'لا رسائل غير مرغوبة، يمكنك إلغاء الاشتراك في أي وقت',
  'Non-binary': 'غير ثنائي',
  'Notify Me': 'أشعرني',
  'Number (0-9)': 'رقم (0-9)',
  'of 4.': 'من 4.',
  'of placement.': 'من وقت الطلب.',
  'on bundle': 'على الباقة',
  'Optional: Tell us what we could improve...': 'اختياري: أخبرنا بما يمكن تحسينه...',
  'Or share via email': 'أو شارك عبر البريد الإلكتروني',
  Order: 'الطلب',
  'Order can be cancelled within': 'يمكن إلغاء الطلب خلال',
  'Order Confirmed': 'تم تأكيد الطلب',
  'Sold Out': 'نفدت الكمية',
  'Total Paid (COD)': 'الإجمالي المستحق (الدفع عند الاستلام)',
  'Skip to content': 'تخطَّ إلى المحتوى',
  'No products yet': 'لا توجد منتجات بعد',
  'Check back soon — new drops land regularly.': 'عد قريبًا — تصل إطلاقات جديدة باستمرار.',
  'Order confirmed and being processed': 'تم تأكيد الطلب وجارٍ معالجته',
  'Order Date': 'تاريخ الطلب',
  'Order not found.': 'الطلب غير موجود.',
  'Order not found. Please check your email and order number.':
    'الطلب غير موجود. تحقق من بريدك ورقم الطلب.',
  'Order total has been updated': 'تم تحديث إجمالي الطلب',
  Orders: 'الطلبات',
  'Orders & Delivery': 'الطلبات والتوصيل',
  Other: 'أخرى',
  'Our Mission': 'مهمتنا',
  'Our Story': 'قصتنا',
  'Password does not meet requirements': 'كلمة المرور لا تفي بالمتطلبات',
  'Password must be at least 8 characters.': 'يجب أن تتكون كلمة المرور من 8 أحرف على الأقل.',
  'Password Requirements': 'متطلبات كلمة المرور',
  'Password Strength': 'قوة كلمة المرور',
  'Password updated successfully': 'تم تحديث كلمة المرور بنجاح',
  'Passwords do not match': 'كلمتا المرور غير متطابقتين',
  'Passwords do not match.': 'كلمتا المرور غير متطابقتين.',
  'Personal Information': 'المعلومات الشخصية',
  Please: 'من فضلك',
  'Please check your code and try again': 'تحقق من الكود وحاول مرة أخرى',
  'Please enter a discount code': 'أدخل كود خصم',
  'Please enter a valid email address.': 'أدخل بريدًا إلكترونيًا صالحًا.',
  'Please enter an email address': 'أدخل بريدًا إلكترونيًا',
  'Please enter both email and order number.': 'أدخل البريد الإلكتروني ورقم الطلب.',
  'Please enter email and order number first.': 'أدخل البريد ورقم الطلب أولاً.',
  'Please refresh the page to try again.': 'حدّث الصفحة وحاول مرة أخرى.',
  'Please select a size': 'اختر مقاسًا',
  'Please select a size for all items': 'اختر مقاسًا لجميع المنتجات',
  'Please select a size.': 'اختر مقاسًا.',
  'Please try again later.': 'حاول مرة أخرى لاحقًا.',
  'Policy pages': 'صفحات السياسات',
  'Postal Code (optional)': 'الرمز البريدي (اختياري)',
  'Prefer not to say': 'أفضل عدم الإفصاح',
  'Previous image': 'الصورة السابقة',
  Price: 'السعر',
  'Privacy Policy': 'سياسة الخصوصية',
  'Processing your request...': 'جارٍ معالجة طلبك...',
  Product: 'المنتج',
  'Product Comparison': 'مقارنة المنتجات',
  'Product Not Found': 'المنتج غير موجود',
  'Product out of stock': 'المنتج غير متوفر',
  'Profile Information': 'بيانات الملف الشخصي',
  'Profile updated successfully': 'تم تحديث الملف الشخصي بنجاح',
  'Quality First': 'الجودة أولًا',
  Quantity: 'الكمية',
  'Questions? Contact us at': 'أسئلة؟ تواصل معنا على',
  'Quick search': 'بحث سريع',
  Rating: 'التقييم',
  'Ready to find your perfect fit?': 'مستعد للعثور على مقاسك المثالي؟',
  'Reason (10-1000 chars, e.g. wrong size)': 'السبب (10-1000 حرف، مثال: مقاس خاطئ)',
  'Reason must be at least 10 characters': 'يجب أن يتكون السبب من 10 أحرف على الأقل',
  Refresh: 'تحديث',
  Remove: 'إزالة',
  'Remove discount': 'إزالة الخصم',
  'Remove from bundle': 'إزالة من الباقة',
  'Remove from wishlist': 'إزالة من المفضلة',
  'Removed from comparison': 'تمت الإزالة من المقارنة',
  'Removed from wishlist': 'تمت الإزالة من المفضلة',
  Request: 'طلب',
  'Request submitted — admin will review within 24h.':
    'تم إرسال الطلب — سيراجعه المشرف خلال 24 ساعة.',
  'Reset filters': 'إعادة تعيين الفلاتر',
  'Resubscribe to Newsletter': 'إعادة الاشتراك في النشرة البريدية',
  return: 'إرجاع',
  Return: 'إرجاع',
  'Return to Store': 'العودة إلى المتجر',
  'Returns & Exchanges': 'الإرجاع والاستبدال',
  'Returns & Sizing': 'الإرجاعات والمقاسات',
  'Review submitted successfully': 'تم إرسال التقييم بنجاح',
  'Review submitted, but some photos failed to upload.':
    'تم إرسال التقييم، لكن بعض الصور فشلت في الرفع.',
  'Save Address': 'حفظ العنوان',
  'Save Changes': 'حفظ التغييرات',
  'Save pieces you love so you can find them here later.': 'احفظ القطع لتحبها لتجدها هنا لاحقًا.',
  'Saving...': 'جارٍ الحفظ...',
  'Saving…': 'جارٍ الحفظ…',
  'Search products': 'ابحث عن منتجات',
  'Search query': 'استعلام البحث',
  'Search tees, hoodies, bags…': 'ابحث عن تيشرتات، هوديز، حقائب…',
  'Search tees, hoodies, denim...': 'ابحث عن تيشرتات، هوديز، دنيم...',
  'Searching…': 'جارٍ البحث…',
  'Secure checkout': 'دفع آمن',
  'Secure checkout — SSL encrypted': 'دفع آمن — مشفّر SSL',
  Select: 'اختر',
  'Select a size': 'اختر مقاسًا',
  'Select gender': 'اختر الجنس',
  'Select governorate': 'اختر المحافظة',
  'Select size': 'اختر المقاس',
  'Send Feedback': 'إرسال الملاحظات',
  'Send Message': 'إرسال رسالة',
  'Send Reset Link': 'إرسال رابط إعادة التعيين',
  'Send Us a Message': 'أرسل لنا رسالة',
  'Set as default': 'تعيين كافتراضي',
  'Set New Password': 'تعيين كلمة مرور جديدة',
  'Share Link': 'مشاركة الرابط',
  'Share link created!': 'تم إنشاء رابط المشاركة!',
  'Share link would be sent to': 'سيُرسل رابط المشاركة إلى',
  'Share this product': 'مشاركة هذا المنتج',
  'Share your thoughts': 'شاركنا آراءك',
  'Share your wishlist': 'شارك قائمة المفضلة',
  'Share Your Wishlist': 'شارك قائمة المفضلة',
  'Shared Wishlist': 'قائمة مفضلة مشتركة',
  'Shipping and taxes calculated at checkout.': 'يُحسب الشحن والضرائب عند إتمام الشراء.',
  'Shop all': 'تسوق الكل',
  'Shop All': 'تسوق الكل',
  'Shop by Category': 'تسوق حسب الفئة',
  'Shop Now': 'تسوق الآن',
  'Short summary': 'ملخص قصير',
  'Sign Out': 'تسجيل الخروج',
  Size: 'المقاس',
  'Size Chart': 'جدول المقاسات',
  Sizes: 'المقاسات',
  "Someone's NERVE Wishlist": 'قائمة مفضلة على NERVE',
  'Something went wrong. Please try again or email us directly.':
    'حدث خطأ ما. حاول مرة أخرى أو راسلنا مباشرة.',
  'Something went wrong. Please try again.': 'حدث خطأ ما. حاول مرة أخرى.',
  'Sort products': 'ترتيب المنتجات',
  'Special character (!@#$%^&*)': 'رمز خاص (!@#$%^&*)',
  'Standard delivery in 2-5 business days across Egypt.':
    'توصيل عادي خلال 2-5 أيام عمل في جميع أنحاء مصر.',
  'Start building your own NERVE wishlist': 'ابدأ في إنشاء قائمة مفضلتك على NERVE',
  'Start by adding products to your comparison from the shop':
    'ابدأ بإضافة منتجات للمقارنة من المتجر',
  'Start Shopping': 'ابدأ التسوق',
  Status: 'الحالة',
  'Stay in the Loop': 'ابقَ على اطلاع',
  'Street, building, apartment': 'الشارع، المبنى، الشقة',
  Subject: 'الموضوع',
  Submit: 'إرسال',
  'Submit Review': 'إرسال التقييم',
  'Submitting...': 'جارٍ الإرسال...',
  Subscribe: 'اشترك',
  'Subscribe Now': 'اشترك الآن',
  'Suggestions:': 'اقتراحات:',
  Sustainability: 'الاستدامة',
  'Tell us a bit about yourself...': 'أخبرنا قليلًا عنك...',
  'Thank you — your NERVE order is being prepared.': 'شكرًا لك — طلبك من NERVE قيد التجهيز.',
  'The New Drop': 'الإصدار الجديد',
  "This page isn't in the closet.": 'هذه الصفحة ليست في الخزانة.',
  'This unsubscribe link is invalid or has expired.':
    'رابط إلغاء الاشتراك هذا غير صالح أو منتهي الصلاحية.',
  'This wishlist has expired or is no longer available.':
    'انتهت صلاحية قائمة المفضلة هذه أو لم تعد متاحة.',
  'Time remaining': 'الوقت المتبقي',
  Title: 'العنوان',
  'to write a review.': 'لكتابة تقييم.',
  'Track Another Order': 'تتبع طلب آخر',
  'Try a broader keyword, clear a filter, or browse our full collection.':
    'جرّب كلمة أوسع، أو امسح فلترًا، أو تصفح مجموعتنا الكاملة.',
  'Try a different search term.': 'جرّب مصطلح بحث مختلف.',
  'Try Again': 'حاول مرة أخرى',
  'Unable to load products. Please try refreshing.': 'تعذّر تحميل المنتجات. حدّث الصفحة.',
  'Unable to search at the moment. Please try again.':
    'تعذّر البحث في الوقت الحالي. حاول مرة أخرى.',
  Up: 'حتى',
  'Update Password': 'تحديث كلمة المرور',
  'Uppercase letter (A-Z)': 'حرف كبير (A-Z)',
  'Use our interactive size calculator to get personalized recommendations':
    'استخدم حاسبة المقاسات التفاعلية للحصول على توصيات مخصصة',
  'Use Size Calculator': 'استخدم حاسبة المقاسات',
  'VAT (14% incl.)': 'ضريبة القيمة المضافة (14% مشمولة)',
  'View All': 'عرض الكل',
  'View Bag': 'عرض الحقيبة',
  'View Full Details': 'عرض التفاصيل الكاملة',
  Waist: 'الخصر',
  'Waist Measurement': 'قياس الخصر',
  'We could not place your order. Please check your connection and try again.':
    'تعذّر تقديم طلبك. تحقق من اتصالك وحاول مرة أخرى.',
  'We recovered your cart and form data.': 'استعدنا سلتك وبيانات النموذج.',
  "We'd love to know why you're leaving:": 'نود أن نعرف سبب مغادرتك:',
  'Welcome back to NERVE.': 'مرحبًا بعودتك إلى NERVE.',
  'Welcome back!': 'مرحبًا بعودتك!',
  'Welcome to NERVE!': 'مرحبًا بك في NERVE!',
  'Welcome to the NERVE family. Watch your inbox for exclusive offers.':
    'مرحبًا بك في عائلة NERVE. تابع بريدك للعروض الحصرية.',
  "We'll email you the moment": 'سنراسلك فور',
  "We've removed": 'قمنا بإزالة',
  'What This Means': 'ماذا يعني هذا',
  'What we collect': 'ما نجمعه',
  'What We Do': 'ما نفعله',
  'Who We Are': 'من نحن',
  'Who we share it with': 'مع من نشارك بياناتك',
  'Why subscribe?': 'لماذا الاشتراك؟',
  'Wishlist Not Found': 'المفضلة غير موجودة',
  'with friends and family': 'مع الأصدقاء والعائلة',
  'Write a Review': 'اكتب تقييمًا',
  'You can only compare up to 3 products': 'يمكنك مقارنة حتى 3 منتجات فقط',
  'Swipe to compare': 'اسحب للمقارنة',
  'View Product': 'عرض المنتج',
  'Try again': 'حاول مرة أخرى',
  "You haven't placed any orders yet.": 'لم تقم بأي طلبات بعد.',
  'You saved': 'وفّرت',
  'You will receive a tracking number via email': 'ستتلقى رقم تتبع عبر البريد الإلكتروني',
  'Your checkout was saved': 'تم حفظ عملية الشراء',
  'Your order is secured with our buyer protection guarantee.': 'طلبك محمي بضمان حماية المشتري.',
  'Your rights': 'حقوقك',
  'Your wishlist is empty': 'قائمة المفضلة فارغة',
  "You're on step": 'أنت في الخطوة',
  "You're on the list. Welcome to NERVE.": 'أنت في القائمة. مرحبًا بك في NERVE.',
  "You've been resubscribed to our newsletter. Expect to hear from us soon!":
    'تمت إعادة اشتراكك في نشرتنا البريدية. توقع سماعنا قريبًا!',
  "You've been unsubscribed": 'تم إلغاء اشتراكك',
  "You've followed the reset link from your email — choose a new password below.":
    'اتبعت رابط إعادة التعيين من بريدك — اختر كلمة مرور جديدة أدناه.',
  Bag: 'الحقيبة',
  'Best Sellers': 'الأكثر مبيعًا',
  'Fan Favorites': 'المفضلة لدى الجميع',
  Browse: 'تصفّح',
  'Shop with confidence': 'تسوّق بثقة',
  'Free shipping over threshold': 'شحن مجاني',
  'Easy returns': 'إرجاع سهل',
  'Find your fit before you order': 'اعثر على مقاسك قبل الطلب',
  'Real people, based in Alexandria': 'فريق حقيقي من الإسكندرية',
  'On Instagram': 'على إنستغرام',
  'Open Instagram': 'فتح إنستغرام',
  'Featured collections': 'تشكيلات مختارة',
  'Slide controls': 'أزرار الشرائح',
  'Go to slide': 'الانتقال إلى الشريحة',
  'Previous slide': 'الشريحة السابقة',
  'Next slide': 'الشريحة التالية',
  'Pause slideshow': 'إيقاف العرض',
  'Play slideshow': 'تشغيل العرض',
  'New Season': 'موسم جديد',
  'The pieces that become part of who you are. Shop the new collection.':
    'القطع التي تصبح جزءًا من من أنت. تسوق المجموعة الجديدة.',
  'Shop New Drop': 'تسوّق الإصدار الجديد',
  'Core Essentials': 'الأساسيات الأساسية',
  'Heavyweight cotton staples you reach for first. Cool but chic, always.':
    'أساسيات قطنية سميكة تختارها أولًا. أنيق لكن أنيق، دائمًا.',
  'Shop Core': 'تسوّق الأساسية',
  'Nerve Archive': 'أرشيف NERVE',
  'Numbered releases. Small batches. Never coming back.': 'إصدارات مرقمة. دفعات صغيرة. لن تعود.',
  'Shop Archive': 'تسوّق الأرشيف',
  'Street Form': 'ستريت فورم',
  'Technical cuts built for the pace of the city.': 'قصّات تقنية مصممة لإيقاع المدينة.',
  'Shop Street Form': 'تسوّق ستريت فورم',
  'Example: NRV-123456': 'مثال: NRV-123456',
  'sign in': 'تسجيل الدخول',
  'Up to': 'حتى',
  Street: 'الشارع',
  building: 'المبنى',
  apartment: 'الشقة',
  "All measurements in centimeters, taken flat. NERVE runs true to size — if you're between sizes, size up for an oversized fit or down for something more fitted.":
    'جميع القياسات بالسنتيمترات وبوضع مستوٍ. NERVE يطابق المقاس المعتاد — إذا كنت بين مقاسين، اختر الأكبر لقصة واسعة أو الأصغر لقصة ضيقة.',
  'NERVE is a contemporary Egyptian concept store built around individuality, movement, and the pieces that become part of your everyday identity. Cool but chic — always.':
    'NERVE متجر مصري معاصر يحتفي بالخصوصية والحركة والقطع التي تصبح جزءًا من هويتك اليومية. أنيق لكن أنيق — دائمًا.',
  'Returns: delivered only, 14 days. Cancellations: placed/processing, 2 hours. One request per order.':
    'الإرجاع: بعد التسليم فقط، خلال 14 يومًا. الإلغاء: عند الطلب/قيد المعالجة، خلال ساعتين. طلب واحد لكل طلبية.',
  'Subscribe to receive exclusive offers, early access to new drops, and fashion inspiration from NERVE.':
    'اشترك لتصلك العروض الحصرية والوصول المبكر للإصدارات الجديدة وإلهام أزياء من NERVE.',
  "Thank you for subscribing to our newsletter. You'll receive exclusive offers and the latest drops directly to your inbox.":
    'شكرًا لاشتراكك في نشرتنا البريدية. ستصلك العروض الحصرية وأحدث الإصدارات مباشرة إلى بريدك.',
  'We use essential cookies to keep your cart and sign-in working. With your permission we also use analytics cookies (Google Analytics / Meta) to improve the store. See our':
    'نستخدم ملفات تعريف ارتباط أساسية لإبقاء سلتك وتسجيل دخولك يعملان. بإذنك نستخدم أيضًا ملفات تحليلية (Google Analytics / Meta) لتحسين المتجر. راجع',
  'Failed to send message': 'فشل إرسال الرسالة',
  'Please enter a subject for your support ticket': 'الرجاء إدخال موضوع لتذكرة الدعم',
  'Support ticket': 'تذكرة الدعم',
  "created! We'll be in touch soon.": 'تم إنشاؤها! سنتواصل معك قريبًا.',
  'Failed to create support ticket. Please try again.': 'فشل إنشاء تذكرة الدعم. حاول مرة أخرى.',
  'Failed to remove item. Please try again.': 'فشل إزالة العنصر. حاول مرة أخرى.',
  'Failed to update quantity. Please try again.': 'فشل تحديث الكمية. حاول مرة أخرى.',
  Added: 'تمت إضافة',
  'to bag': 'إلى الحقيبة',
  'Bundle added!': 'تمت إضافة الباقة!',
  'off applied with': 'خصم مطبّق بكود',
  'you saved': 'وفّرت',

  // PDP upgrade
  'Back to Shop': 'العودة إلى المتجر',
  'This product may have been removed or the link is wrong.':
    'ربما تمت إزالة هذا المنتج أو الرابط غير صحيح.',
  'This product is sold out.': 'نفدت كمية هذا المنتج.',
  'That size is no longer available.': 'هذا المقاس لم يعد متاحًا.',
  'Could not add to bag. Please try again.': 'تعذّرت الإضافة إلى الحقيبة. حاول مرة أخرى.',
  'Added to Bag': 'تمت الإضافة إلى الحقيبة',
  'Zoom product image': 'تكبير صورة المنتج',
  'Product image zoom': 'تكبير صورة المنتج',
  'Close zoom': 'إغلاق التكبير',
  'View product image': 'عرض صورة المنتج',
  thumbnail: 'مصغّرة',
  view: 'عرض',
  'Select color': 'اختر اللون',
  Available: 'متاح',
  Unavailable: 'غير متاح',
  'True to size — check the size guide for measurements.':
    'يطابق المقاس المعتاد — راجع دليل المقاسات للقياسات.',
  Description: 'الوصف',
  'Details & Care': 'التفاصيل والعناية',
  'Shipping & Returns': 'الشحن والإرجاع',
  'Fit:': 'القصة:',
  'No care instructions listed.': 'لا توجد تعليمات عناية مدرجة.',
  'Open Size Guide': 'فتح دليل المقاسات',
  'Express delivery:': 'التوصيل السريع:',
  'Free standard shipping on orders over': 'شحن عادي مجاني للطلبات فوق',
  'Orders process within 24 hours on business days.': 'تُعالَج الطلبات خلال 24 ساعة في أيام العمل.',
  'Full shipping policy': 'سياسة الشحن الكاملة',
  'Free size exchanges within 30 days.': 'تبديل المقاسات مجانًا خلال 30 يومًا.',
  'Full returns policy': 'سياسة الإرجاع الكاملة',
  'pay the courier when your order arrives across Egypt.':
    'ادفع للمندوب عند وصول طلبك في جميع أنحاء مصر.',
  'We never collect or store payment card details.':
    'لا نجمع أو نخزّن بيانات بطاقات الدفع إطلاقًا.',
  '14-day returns on unworn items': 'إرجاع خلال 14 يومًا للمنتجات غير المستخدمة',
  'Store policies': 'سياسات المتجر',
  'No reviews yet': 'لا تقييمات بعد',
  reviews: 'تقييمات',
  by: 'بواسطة',
  Anonymous: 'مجهول',
  Uploaded: 'مرفوعة',
  'Loading reviews': 'جارٍ تحميل التقييمات',
  'Sort reviews': 'ترتيب التقييمات',
  'Questions?': 'أسئلة؟',
  'NERVE fits true to size unless noted. If between sizes, size up for an oversized look or down for a fitted silhouette.':
    'NERVE يطابق المقاس المعتاد ما لم يُذكر خلاف ذلك. إذا كنت بين مقاسين، اختر الأكبر لإطلالة واسعة أو الأصغر لقصة ضيقة.',

  // Discovery / search & filters
  'Active filters': 'عوامل التصفية النشطة',
  Availability: 'التوفر',
  'Best Selling': 'الأكثر مبيعًا',
  'In stock only': 'المتوفر فقط',
  'Including close matches for': 'بما فيها النتائج المشابهة لـ',
  Newest: 'الأحدث',
  'No products match your filters': 'لا توجد منتجات تطابق عوامل التصفية',
  'Price: Low to High': 'السعر: من الأقل إلى الأعلى',
  'Price: High to Low': 'السعر: من الأعلى إلى الأقل',
  'Remove filter': 'إزالة عامل التصفية',
  'Search suggestions': 'اقتراحات البحث',
  'Search the shop for': 'ابحث في المتجر عن',
  'View all results': 'عرض كل النتائج',
  results: 'نتيجة',
};
