...
export default function CustomerDetail() {
  ...
  useEffect(() => {
    ...
    adminService
      ...
      .catch(err => logError('Failed to load customer', err));
    adminService
      .listCustomerOrders(id)
      .then(o => setOrders(o))
      .catch(err => logError('Failed to load customer orders', err));
  }, [id]);
  ...
  return (
    <AdminLayout>
      <h1 className="nv-heading text-3xl mb-6">
      ...
      <section className="mb-6">
        ...
        <div className="border border-navy/10 p-4">
          ...
          <p>
            <strong>Joined:</strong> {new Date(customer.created_at).toLocaleString()}
          </p>
        </div>
      </section>
            ...
            {customer.customer_addresses.map((a: any) => (
              <li key={a.id} className="border border-navy/10 p-3">
                ...
                <div className="text-xs text-navy/70">
                  {a.address}, {a.city}, {a.governorate} {a.postal_code ?? ''}
                </div>
              </li>
            ))}
          </ul>
