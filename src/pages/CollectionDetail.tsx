...
export default function CollectionDetail() {
  ...
  useEffect(() => {
    ...
    let mounted = true
    setLoading(true)
    Promise.all([productService.getCollection(id), productService.getProductsByCollection(id)]).then(
      ([c, p]) => {
        if (!mounted) return
  ...
  if (!collection) {
    return (
      <div className="bg-white text-navy min-h-screen pt-32 px-5 text-center">
        ...
        <Link to="/collections" className="nv-eyebrow underline">Back to Collections</Link>
      </div>
    )
  }
